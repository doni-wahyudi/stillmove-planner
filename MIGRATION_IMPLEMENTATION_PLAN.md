...
| 9 | completed | Event Planner Suite for Event Organizers | Full-featured EO suite: Rundown cue sheet with live stage monitor & print mode, committee crew roster with 1-tap WhatsApp links, budget RAB with payment states, logistics master pipeline, vendor directory, milestone checklists, template import/export, and Supabase SQL schema. |
...
| 2026-08-23 | 9 | Completed. Event Planner suite implemented for Event Organizer teams: Multi-event dashboard with D-Day countdown banner, minute-by-minute Rundown with Live Stage Monitor mode & Print Cue Sheet, committee Crew roster with 1-tap WhatsApp quick-links, Budget RAB with payment status & variance tracking, Logistics master pipeline (needed ➔ sourced ➔ on-site ➔ returned), Vendors & Talent directory with call times, phase-by-phase Milestones checklist, JSON template import/export, and complete Supabase SQL migration schema (supabase_events_schema.sql). TypeScript check passes (exit 0). |
